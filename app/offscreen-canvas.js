// use global scope since this code may be imported by a worker
self.OffscreenPxCanvas = class OffscreenPxCanvas extends EventTarget {
  ...
  #z;
  #gridSize = 0;
  #maxColors = OffscreenPxCanvas.MAX_SUPPORTED_COLORS;
  #dithering = 1;
  ...
  async setImage(image) {
    ...
    this.dispatchEvent(new CustomEvent('maxGridSize', { detail: maxGridSize }));

    // reset settings before `initImage` is called
    this.#gridSize = 0;
    this.#maxColors = OffscreenPxCanvas.MAX_SUPPORTED_COLORS;
    this.#dithering = 1;
    ...
    const maxColors = Math.min(
      this.#colorMap.size,
      OffscreenPxCanvas.MAX_SUPPORTED_COLORS,
    );
    this.dispatchEvent(new CustomEvent('maxColors', { detail: maxColors }));
  }
  ...
  zoom({ scaleDiff, x, y }) {
    ...
    this.dispatchEvent(new CustomEvent('zoom', { detail: this.#z }));
  ...
  *#labeledPixels() {
  ...
  #renderColors() {
    this.dispatchEvent(new CustomEvent('colors', { detail: this.#colorMap }));
  }
};
